...
_jit_block_name(LogFilterXPipe *self)
{
  static gchar block_name[1024];
  g_snprintf(block_name, sizeof(block_name), "block_%s", self->name);
...
_compile_block(LogFilterXPipe *self, GlobalConfig *cfg)
{
  ...
  filterx_jit_ir_add_new_block(jit, _jit_block_name(self));
...
_setup_jit_exec(LogPipe *s)
{
  ...
  GError *error = NULL;
  FilterXJITAddress addr = filterx_jit_lookup(jit, _jit_block_name(self), &error);
  if (!addr)
    {
