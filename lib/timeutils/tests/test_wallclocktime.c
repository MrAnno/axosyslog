...
Test(wallclocktime, test_wall_clock_time_init)
{
  ...
  cr_assert(not(wall_clock_time_is_set(&wct)));
}
...
Test(wallclocktime, test_strptime_parses_broken_down_time)
{
  ...
  cr_assert(eq(chr, *end, 0));
  cr_expect(eq(int, wct.wct_year, 119));
  cr_expect(eq(int, wct.wct_mon, 0));
  cr_expect(eq(int, wct.wct_mday, 16));

  cr_expect(eq(int, wct.wct_hour, 18));
  cr_expect(eq(int, wct.wct_min, 23));
  cr_expect(eq(int, wct.wct_sec, 12));
  cr_expect(eq(int, wct.wct_usec, 12345));
...
Test(wallclocktime, test_strptime_percentf_fractions_are_optional)
{
  ...
  cr_assert(eq(chr, *end, 0));
  cr_expect(eq(int, wct.wct_usec, 0));

  /* something else follows */
  wct.wct_usec = 999999;
  end = wall_clock_time_strptime(&wct, "%S%f %z", "12 +02:00");
  cr_assert(eq(chr, *end, 0), "%s", end);
  cr_expect(eq(int, wct.wct_usec, 0));
  cr_expect(eq(long, wct.wct_gmtoff, 7200));
}
...
Test(wallclocktime, test_strptime_parses_truncated_usec)
{
  ...
  cr_assert(eq(chr, *end, 'X'));
  cr_expect(eq(int, wct.wct_usec, 12000));
}
...
Test(wallclocktime, test_strptime_usec_parse_finds_character)
{
  ...
  cr_assert(eq(chr, *end, 0));
  cr_expect(eq(int, wct.wct_usec, 0), "%d", wct.wct_usec);
}
...
Test(wallclocktime, test_strptime_percent_z_parses_rfc822_timezone)
{
  ...
  cr_expect(eq(long, wct.wct_gmtoff, -8*3600), "Unexpected timezone offset: %ld, expected -8*3600", wct.wct_gmtoff);
  ...
  cr_expect(eq(long, wct.wct_gmtoff, -4*3600), "Unexpected timezone offset: %ld, expected -4*3600", wct.wct_gmtoff);

  end = wall_clock_time_strptime(&wct, "%b %d %Y %H:%M:%S %z", "Jan 16 2019 18:23:12 GMT");
  cr_expect(eq(long, wct.wct_gmtoff, 0), "Unexpected timezone offset: %ld, expected 0", wct.wct_gmtoff);
  ...
  cr_expect(eq(long, wct.wct_gmtoff, 1*3600), "Unexpected timezone offset: %ld, expected 1*3600", wct.wct_gmtoff);
  ...
  cr_expect(eq(long, wct.wct_gmtoff, -12*3600), "Unexpected timezone offset: %ld, expected -12*3600", wct.wct_gmtoff);
  wct.wct_gmtoff = -1;
  ...
  cr_expect(zero(ptr, end));

  /* hours only */
  wct.wct_gmtoff = -1;
  end = wall_clock_time_strptime(&wct, "%b %d %Y %H:%M:%S %z", "Jan 16 2019 18:23:12 +05");
  cr_expect(eq(long, wct.wct_gmtoff, 5*3600), "Unexpected timezone offset: %ld, expected 5*3600", wct.wct_gmtoff);
  ...
  cr_expect(eq(long, wct.wct_gmtoff, 5*3600+30*60), "Unexpected timezone offset: %ld, expected 5*3600+30*60",
            wct.wct_gmtoff);
}
...
Test(wallclocktime, test_strptime_percent_Z_allows_timezone_to_be_optional)
{
  ...
  cr_expect(not(zero(ptr, end)));
  cr_expect(eq(str, end, " PST"));
  ...
  cr_expect(eq(long, wct.wct_gmtoff, -1), "Unexpected timezone offset: %ld, expected -1", wct.wct_gmtoff);
...
Test(wallclocktime, test_strptime_percent_Z_numeric_formats)
{
  ...
  cr_expect(eq(chr, *end, '\0'));
  cr_expect(eq(long, wct.wct_gmtoff, 2*3600), "Unexpected timezone offset: %ld, expected 2*3600", wct.wct_gmtoff);
...
Test(wallclocktime, test_strptime_zone_parsing_takes_daylight_saving_into_account_when_using_the_local_timezone)
{
  ...
  cr_assert(eq(chr, *end, 0));
  cr_expect(eq(int, wct.wct_year, 121));
  cr_expect(eq(int, wct.wct_mon, 4));
  cr_expect(eq(int, wct.wct_mday, 7));

  cr_expect(eq(int, wct.wct_hour, 9));
  cr_expect(eq(int, wct.wct_min, 29));
  cr_expect(eq(int, wct.wct_sec, 12));
  cr_expect(eq(int, wct.wct_usec, 0));

  cr_expect(gt(int, wct.wct_isdst, 0));
  cr_expect(eq(long, wct.wct_gmtoff, 2*3600), "Unexpected timezone offset: %ld, expected 2*3600", wct.wct_gmtoff);
  ...
  cr_assert(eq(chr, *end, 0));
  cr_expect(eq(int, wct.wct_year, 121));
  cr_expect(eq(int, wct.wct_mon, 1));
  cr_expect(eq(int, wct.wct_mday, 7));
  ...
  cr_expect(eq(int, wct.wct_isdst, 0));
  cr_expect(eq(long, wct.wct_gmtoff, 1*3600), "Unexpected timezone offset: %ld, expected 1*3600", wct.wct_gmtoff);
...
Test(wallclocktime, guess_year_returns_last_year)
{
  ...
  cr_assert(eq(chr, *end, 0));
  cr_assert(eq(int, wct.wct_year, -1));
...
Test(wallclocktime, test_strptime_parses_without_mday)
{
  ...
  wall_clock_time_guess_missing_fields(&wct);
  cr_expect(not(zero(ptr, end)));
  cr_expect(eq(int, wct.wct_mday, 1));
}
...
Test(wallclocktime, test_strptime_parses_without_time)
{
  ...
  wall_clock_time_guess_missing_fields(&wct);
  cr_expect(not(zero(ptr, end)));
  cr_expect(eq(int, wct.wct_hour, 0));
  cr_expect(eq(int, wct.wct_min, 0));
  cr_expect(eq(int, wct.wct_sec, 0));
}
...
Test(wallclocktime, test_strptime_parses_without_second)
{
  ...
  wall_clock_time_guess_missing_fields(&wct);
  cr_expect(not(zero(ptr, end)));
  cr_expect(eq(int, wct.wct_min, 30));
  cr_expect(eq(int, wct.wct_sec, 0));
}
...
Test(wallclocktime, test_strptime_percent_z_is_mandatory)
{
  ...
  WallClockTime wct = WALL_CLOCK_TIME_INIT;
  cr_assert(zero(ptr, wall_clock_time_strptime(&wct, "%Y-%m-%d %T%z", "2011-06-25 20:00:04")));
}
...
Test(wallclocktime, test_strftime_percent_f)
{
  ...
  cr_assert(eq(str, buf, ".123456"));
  wall_clock_time_strftime(&wct, buf, sizeof(buf), ".%3f");
  cr_assert(eq(str, buf, ".123"));
  ...
  wall_clock_time_strftime(&wct, buf, sizeof(buf), ".%f");
  cr_assert(eq(str, buf, ".012345"));
  wall_clock_time_strftime(&wct, buf, sizeof(buf), ".%3f");
  cr_assert(eq(str, buf, ".012"));
  wall_clock_time_strftime(&wct, buf, sizeof(buf), ".%6f");
  cr_assert(eq(str, buf, ".012345"));
  wall_clock_time_strftime(&wct, buf, sizeof(buf), ".%9f");
  cr_assert(eq(str, buf, ".012345"));
...
Test(wallclocktime, test_strftime_can_be_parsed_by_strptime)
{
  ...
  cr_expect(eq(int, wct.wct_year, wct2.wct_year));
  cr_expect(eq(int, wct.wct_mon, wct2.wct_mon));
  cr_expect(eq(int, wct.wct_mday, wct2.wct_mday));

  cr_expect(eq(int, wct.wct_hour, wct2.wct_hour));
  cr_expect(eq(int, wct.wct_min, wct2.wct_min));
  cr_expect(eq(int, wct.wct_sec, wct2.wct_sec));
  cr_expect(eq(int, wct.wct_usec, wct2.wct_usec));

  cr_expect(eq(int, wct.wct_isdst, wct2.wct_isdst), "%d != %d", wct.wct_isdst, wct2.wct_isdst);
  cr_expect(eq(long, wct.wct_gmtoff, wct2.wct_gmtoff));
}
...
Test(wallclocktime, test_strptime_seconds_since_epoch_timezone)
{
  ...
  cr_expect(eq(long, wct.wct_gmtoff, wct2.wct_gmtoff));
  cr_expect(eq(int, wct.tm.tm_isdst, wct2.tm.tm_isdst));
  cr_assert(eq(str, wct.wct_zone, wct2.wct_zone));
}
