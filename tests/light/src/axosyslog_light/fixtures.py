...
def pytest_addoption(parser):
    ...
    parser.addoption("--disable-filterx-jit", action="store_true", help="Disable JIT compilation of filterx expressions.")
...
def config(request, syslog_ng, teardown) -> SyslogNgConfig:
    jit_enabled = request.getfixturevalue("jit_available") and not request.config.getoption("--disable-filterx-jit")
    return syslog_ng.create_config(request.getfixturevalue("config_version"), teardown, jit_enabled)
